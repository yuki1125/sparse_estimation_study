...
今は$\beta_j$に対して微分しているため、上式では\sum_{j=1}^p\beta_i x_{i,j}$の部分は、$j$以外の要素は消えていることに注意。

ここで簡単のため以下の仮定を置く。（一般化は問題8で行う。ここでは原理確認）

余談：Markdown形式の数式でも数式に番号をつけられることに気がついた。以降、番号をつける。以前の数式にも時間があればつける。
$$
\tag{6.3}
...
$\beta_j=$の形に変形する。例えば
$-s_j+\beta_j+\lambda=0$なので、$\beta_j=s_j-\lambda$であり、この時$\beta_j>0$であるため、$s_j >\lambda$が成り立つ必要がある。
